...
- **WorldGuard/WorldEdit/Towny: attempted bumps reverted, staying on `7.0.5`/`7.2.0`/`0.101.1.0`.** Source-level API diffing (same method as above) showed no incompatible usage for `7.0.18`/`7.4.5`/`0.103.2.0`, but CI failed to even compile against them: those published artifacts are class files built with newer JDKs than this project's JDK 16 toolchain can read (`class file has wrong version 69.0, should be 60.0` — Java 25 bytecode — for WorldGuard/WorldEdit; `63.0`/Java 19 for Towny). This is a JDK-level constraint, not an API-compatibility one, and it isn't specific to any one of these three versions — it's coupled to this project's own Java target, which the separate Spigot/Paper platform-version update is expected to raise. Left at their prior versions until that lands; revisit then.
